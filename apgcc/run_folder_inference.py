...
from datasets.build import collate_fn_crowd
from models import build_model
from util.logger import vis

SCRIPT_DIR = Path(__file__).resolve().parent
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "output"
DEFAULT_VISUALIZATION_DIR = DEFAULT_OUTPUT_DIR / "visualization"
DEFAULT_CHECKPOINT = DEFAULT_OUTPUT_DIR / "SHHA_best.pth"
DEFAULT_CONFIG = SCRIPT_DIR / "configs" / "SHHA_test.yml"
DEFAULT_WORKING_DIR = SCRIPT_DIR / "custom_data"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--visualize",
        action="store_true",
        help="Save visualizations that overlay the predicted points on each image.",
    )
    parser.add_argument(
        "--vis_dir",
        type=Path,
        ...
        help=(
            "Directory where visualization images will be written."
            " Defaults to OUTPUT_DIR/folder_inference_vis when --visualize is set."
        ),
    )
    parser.add_argument(
        "--materialize_dataset",
        action="store_true",
        help=(
            "Persist a synthetic SHHA dataset to disk before running inference. "
            "When omitted, everything runs in-memory without duplicating images."
        ),
    )
...
def prepare_shha_style_dataset(
    ...
    resolved_paths = list(image_paths) if image_paths is not None else collect_image_paths(images_dir)
...
class InMemoryCrowdDataset(Dataset):
    """Dataset that generates synthetic SHHA-style targets without touching disk."""
    ...
    def __init__(
        ...
        self.synthetic_points: List[torch.Tensor] = []
        rng = random.Random(seed)
        for image_path in self.image_paths:
            ...
            coords = [
                [
                    rng.uniform(0, max(width - 1, 1)),
                    rng.uniform(0, max(height - 1, 1)),
                ]
                for _ in range(num_points)
            ]
            if coords:
                ...
                tensor = torch.empty((0, 2), dtype=torch.float32)
            self.synthetic_points.append(tensor)
    ...
    def __getitem__(self, index: int):
        ...
        points = self.synthetic_points[index]
        labels = torch.ones(points.shape[0], dtype=torch.long, device=points.device)
        target = [
            {
                "point": points,
...
def build_in_memory_loader(
    ...
    dataset = InMemoryCrowdDataset(
        image_paths=image_paths,
        min_random_points=min_random_points,
        max_random_points=max_random_points,
        seed=seed,
    )
    return DataLoader(
        dataset,
...
def predict_counts(
    ...
    results = []
    model.eval()
    vis_path = str(vis_dir) if vis_dir is not None else None
    with torch.no_grad():
        for samples, targets in data_loader:
            ...
            points_tensor = outputs['pred_points'][0][keep]
            points_list = points_tensor.detach().cpu().tolist()
            ...
            if vis_path is not None:
                vis(samples.detach().cpu(), targets, [points_list], vis_path)
    return results
...
def main():
    ...
    if args.vis_dir is not None and not args.visualize:
        print("Warning: --vis_dir is ignored unless --visualize is set.")
    ...
    if args.materialize_dataset:
        ...
        dataset_for_cfg = dataset_root
        print(f"Prepared synthetic SHHA dataset with {len(entries)} images at {dataset_root}.")
    else:
        dataset_for_cfg = images_dir
        print(
            "Running in-memory inference on %d images without materializing SHHA files."
            % len(image_paths)
        )
    ...
    if args.visualize:
        if args.vis_dir is not None:
            ...
            vis_dir = output_dir / f"folder_inference_vis_{args.threshold:.2f}"
        vis_dir.mkdir(parents=True, exist_ok=True)
        print(f"Saving visualization overlays to {vis_dir}")
    ...
    if args.materialize_dataset:
        ...
        val_loader = build_in_memory_loader(
            image_paths,
            args.min_random_points,
            args.max_random_points,
            args.seed,
